...
from tools.base import basedir
from fake_useragent import UserAgent
from requests.exceptions import RequestException
from tools.base import r
...
class Tools(object):
    ...
    def log(self, event):
        ...
        with open(log_name, 'a+', encoding='utf8') as f:
            f.write('%s%s\n' % (str(datetime.datetime.now()), event))
        print('%s%s\n' % (str(datetime.datetime.now()), event))
    ...
    def get_html_req(self, url):
        ...
        try:
            res = requests.get(url, timeout=10, headers=self.randHeader(), cookies=self.cookies, proxies=self.proxies)
    ...
    def get_int(self, txt):
        req = re.search('\d+', txt)
        if req is not None:
            return int(req.group())
        else:
            return 0

    def get_time(self, txt):
        reg = '\d+\.\d+\.\d+\s\d+\:\d+'
        req = re.search(reg, txt)
        if req is not None:
            return str(req.group())
        else:
            return ""

    def get_star(self, txt):
        try:
            if txt == '':
                return txt
            reg = 'star(\d)\.gif'
